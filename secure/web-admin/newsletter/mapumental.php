...
if ($text) {
	...
	if ($LIVE && $_POST['send-test']) {
		$email = OPTION_NEWSLETTER_MAPU_TEST_LIST;
		$html = call("perl create-mapu-preview email-html $email", "$subject\n\n$text");
		print "<p><strong>Test email sent</strong></p>";
	}
	if ($LIVE && $_POST['send-mapu']) {
		$email = OPTION_NEWSLETTER_MAPU_LIVE_LIST;
		$html = call("perl create-mapu-preview email-html $email", "$subject\n\n$text");
		print "<p><strong>Mapumental email sent</strong></p>";
	}
	$html = call('perl create-mapu-preview html', "$subject\n\n$text");
	print "<h2>HTML Preview</h2> <div style='width: 640px; border: solid 1px #999999;'>$html</div>";
	$plain = call('perl create-mapu-preview plain', "$subject\n\n$text");
...
<input type='submit' name="send-mapu" value='Send to Mapumental list' onclick="return confirm('Are you sure?');">
<? } ?>
</p>
</form>
